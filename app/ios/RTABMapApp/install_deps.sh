...
# IXWebsocket
if [ ! -e $prefix/include/ixwebsocket ]
then
if [ ! -e ixwebsocket ]
then
...
if [ ! -e $prefix/include/google/protobuf ]
then
  if [ ! -e protobuf ]
  then
    git clone https://github.com/protocolbuffers/protobuf.git
    cd protobuf
    git checkout v29.1  # Explicitly use version 29.1
...
#   git clone https://github.com/protocolbuffers/protobuf.git
#   cd protobuf
#   # git checkout v3.19.4  # Explicitly use version 3.19.4
#   # git submodule update --init --recursive
#   cd $pwd
#   cd protobuf
# else 
...
# # Install using cmake --install instead of --build --target install
# cmake --install . --config Release
#  cmake --build . --config Release --target install 
# cd $pwd
# #rm -rf protobuf
